		...
		<movable-area>
			...
			<!-- #endif -->
				...
				<view class="tabstop flex-center">
					<u-tabs :list="tabslisttop" :current="tabsactivetop" @click="tabstophandle" lineColor="#1B1BDD" 
					activeStyle="color:#1B1BDD; font-size: 17px; font-weight: 700;font-family: 'PingFang SC Bold';}"
					inactiveStyle="color: #373748; font-size: 17px; font-weight: 700;font-family: 'PingFang SC Bold';" lineWidth="15"
					itemStyle="padding-left: 15px; padding-right: 15px; height: 44px; font-size: 36upx;"></u-tabs>
				</view>
				<!-- 轮播 -->
				<AppBanner :bannerList="bannerList" previousMargin="0" nextMargin="0"></AppBanner>
				<!-- 标签 -->
				<view class="tabs" v-if="this.tabsactivetop == 0 || this.tabsactivetop == 1">
					<u-tabs :list="tabslist" :current="tabsactive" @click="tabshandle" lineColor="#1B1BDD" activeStyle="color:#373748; font-weight: 700;" 
					inactiveStyle="color: #666666" lineWidth="30" itemStyle="padding-left: 12px; padding-right: 12px; height: 44px;"></u-tabs>
				</view>
				...
				<block v-if="!pageLoading">
					<view class="bottom" v-if="list && list.length">
						...
						<!-- 快讯 -->
						<realimeinfo v-if="tabsactivetop==1" :list="list" :tabsactivetop="tabsactivetop"></realimeinfo>
						<!-- KOL言论 -->
						<realimeinfo v-if="tabsactivetop==2" :list="list" tab="kol" @acticlehandle="acticlehandle" @integralhandle="integralhandle"></realimeinfo>
					</view>
...
export default {
	...
	data() {
		return {
			...
			tabslisttop:[
				...
				{id: 1,name: '快讯',type:'newsflash'},
				{id: 2,name: 'KOL言论',type: 'kol'},
			],
			tabsactivetop: 0,
			tabsactivetype: 'article',
			tabslist: [],
			tabsactive: 0,
			category_id: '',
			category_name: '',
	...
	methods: {
		...
		//banner数据
		getbannerList(){
			this.$http('banner', {is_page: 0,code:'realinfo'}, 'get').then((res) => {
