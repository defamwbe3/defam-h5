<template>
	<view class="comment-wrap">
		<view class="title flex-between">
			<view class="left">评论{{commentlist.length||0}}</view>
			...
			<view class="items" v-for="(item,index) in commentlist" :key="index">
				<view class="information flex-between">
					...
					<view class="flex-middle">
						<image class="deteimage" @click="delect(item.id)" v-if="item.member.id == userinfo.id" :src="IMG_URL+'icon_shanchu.png'" mode=""></image>
				...
				if(uni.getStorageSync('token')){
					...
					uni.showModal({
						...
						success: function (res) {
							if (res.confirm) {
								uni.showLoading()
								_this.$http('comment/del', {id: id}, 'get').then(res=>{
									uni.hideLoading()
									setTimeout(()=>{
										_this.$u.toast(res.message)
									})
									_this.$emit('commenthandle',{pinglun: true})
									_this.page = 1
									_this.commentinfolisthandle()
								}).catch(()=>{})
							}
						}
					})
				}else{
					_this.$store.commit('IS_LOGIN', true);
				}
			},
			//收藏与取消
			is_collectionhandle(){
